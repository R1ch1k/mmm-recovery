...
@pytest.mark.xfail(
    ...
    reason=(
        ...
        "to adjust. MEASURED with G1's own metric (median |relative bias| across channels, "
        "threshold 20%): 0.540 on C0 at 200 seeds. At the exactly true hyperparameters it is "
        "EXACT once D22's controls are in place, so the estimand is recoverable and the "
        "failure is entirely in choosing the transform. "
        ...
        "(alpha, kappa) on a 26x30 grid, REGENERATED at D39 as the committed module "
        "mmm_recovery.plateau after the original harness was found never to have been "
        "committed: on the NOISY series 639 of 780 grid points sit within 1% of the truth's "
        "CV RMSE of 30.94153, 116 of 780 fit strictly BETTER than the truth, the best-fitting "
        "point on the whole grid is 42.8% wrong (29,757 against a true 51,989 GBPk), and TV's "
        "contribution across the near-tied set ranges 15,138 to 248,075 GBPk -- a 16.4x "
        "spread, all indistinguishable to the objective. The NOISELESS arm is the contrast "
        "that pins the mechanism: with D22's controls the truth scores 0.00002 against a best "
        "competitor of 0.051795, 2,590x worse, and that runner-up is only 3.3% wrong, so the "
        "form is identifiable in principle and is not identifiable from this data. Noise of "
        "sd 29.32 GBPk/wk against a media series of sd 16.8 erases the curvature the level is "
        "identified through. These figures REPLACE the pre-D39 '177 of 780, 43,938 to 240,522 "
        "GBPk, 5.5x' quoted here until now: that sweep reported a truth CV of 3.63160 with "
        "2.4% bias, which matches neither current arm and is the size of the structured "
        "residual D22 records for the SUPERSEDED six-column control block, so the published "
        "centrepiece described a configuration the study had abandoned. The regeneration was "
        "pre-committed to be authoritative and the sweep was not tuned to recover 177 (D39). "
        "That is 'attributable != incremental' stated as a measurement. "
        ...
        "The failure is NOT confined to this noiseless build-order test, and D23 settled it "
        "on the full seed count with D22's controls in place: on the noisy C0 cell the grid "
        "actually runs, at 200 seeds, ALL FIVE gates fail -- G1 0.540, G2 0.417, G3 0.650, "
        "G4 2.355, G5 0.200. Nine of 200 seeds pass G1 individually and 160 of 200 produce "
        "regret above 100%, meaning the advice is worse than not acting. Under noise CV RMSE "
        "is 29.1-32.3 against a noise sd of 29.32 GBPk/wk, so the criterion being minimised "
        "is over 99% noise variance. "
        "K3 remedies attempted and refuted. (1) Tighter regularisation is monotonically "
        "worse, and this is now measured across 10 seeds on the NOISY series rather than on "
        "one noiseless seed: 0.640 at the CV-chosen penalty, 0.509 at 1e-6, 0.515 at 1e-4, "
        "0.557 at 1e-2, 0.893 at 1e-1, 0.987 at 1.0. No penalty passes G1. Ridge shrinks the "
        "media block toward zero and non-negativity is already enforced. (3) A demand proxy "
        "is inapplicable at C0, where gamma=0 and phi=0 and there is no confounding to fix. "
        "(2) Shrinkage to an informative prior ROAS is a Step 7 item; rescuing the HARNESS "
        "gate with a prior would mean C0 validates only when the estimator is told the "
        "answer. "
        "An earlier version of this reason quoted 13.8% as the remedy-1 baseline. That was "
        "(noiseless, one draw stream, seed 0) -- the minimum of ten values whose median is "
        "0.526 -- quoted as though it were a configuration. Recorded because it is the same "
        "selection-on-noise error that produced the first wrong diagnosis of this failure. "
        "The additive-vs-product control gap, once the leading secondary suspect, was FIXED "
        "by D22 rather than argued away: the residual is now 1.4e-13 and recovery at the true "
        "hyperparameters is exact. It moved the failure 0.641 -> 0.640 under noise. That it "
        "is fixed and the gates still fail is what licenses a structural reading."
    ),
)
